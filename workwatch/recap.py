...
# Most-active project groups to show in the "Explored" section; the rest are
# collapsed into a one-line tail so a long list of one-off sessions doesn't
# bury the projects you actually spent time in.
MAX_SESSION_GROUPS = 8
...
def build_plain_body(data: dict, full: bool = False) -> str:
    ...
    if not sess["available"]:
        ...
        if not groups:
            ...
            if tail:
                tail_sessions = sum(g["count"] for g in tail)
                L.append(f"  ⋯ +{len(tail)} more project(s) · {tail_sessions} session(s)")
    ...
    if not sess["available"]:
        ...
        if not groups:
            ...
            if tail:
                tail_sessions = sum(g["count"] for g in tail)
                rows.append(
                    f'<div style="color:#aaa;font-size:12px;padding:8px 0;">'
                    f'⋯ +{len(tail)} more project(s) · {tail_sessions} session(s)</div>'
                )
            sess_html = "".join(rows)
